...
bool write_webm_block(struct WebmOutputContext *webm_ctx,
                      const vpx_codec_enc_cfg_t *cfg,
...
//                   cfg->g_timebase.den;
//make it pts pass count by "millisecond".
  int64_t pts_ns = pkt->data.frame.pts * 1000000ll * cfg->g_timebase.num /
                   cfg->g_timebase.den;
  if (pts_ns <= webm_ctx->last_pts_ns) pts_ns = webm_ctx->last_pts_ns + 1000000;
  webm_ctx->last_pts_ns = pts_ns;

  return segment->AddMetadata(static_cast<uint8_t *>(pkt->data.frame.buf),
